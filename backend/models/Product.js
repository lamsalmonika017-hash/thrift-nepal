...
const Product = {
  create: async (data) => {
    const [result] = await db.query(
      'INSERT INTO products (title, description, category, price, `condition`, seller_id) VALUES (?, ?, ?, ?, ?, ?)',
      [data.title, data.description, data.category, data.price, data.condition, data.seller_id]
    );
    return result;
  },
  ...
  getAll: (filters = {}) => {
    ...
    if (filters.search) { sql += ' AND (p.title LIKE ? OR p.description LIKE ?)'; params.push(`%${filters.search}%`, `%${filters.search}%`); }
  ...
  getFeatured: () => db.query(`
    ...
    WHERE p.status = 'available' AND p.is_featured = 1 ORDER BY p.created_at DESC LIMIT 8
  `),
  ...
  getTrending: () => db.query(`
    SELECT p.*, u.username as seller_name,
      (SELECT image_url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as primary_image
    FROM products p LEFT JOIN users u ON p.seller_id = u.id
    WHERE p.status = 'available' ORDER BY p.views DESC LIMIT 8
  `),
  ...
  getRecent: () => db.query(`
    SELECT p.*, u.username as seller_name,
      (SELECT image_url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as primary_image
    FROM products p LEFT JOIN users u ON p.seller_id = u.id
    WHERE p.status = 'available' ORDER BY p.created_at DESC LIMIT 12
  `),

  getById: async (id) => {
    const [products] = await db.query(`
      SELECT p.*, u.username as seller_name, u.college as seller_college, u.phone as seller_phone, u.avatar as seller_avatar
  ...
  getByCategory: (category, limit = 8) => db.query(`
    SELECT p.*, u.username as seller_name,
      (SELECT image_url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as primary_image
    FROM products p LEFT JOIN users u ON p.seller_id = u.id
    WHERE p.status = 'available' AND p.category = ? ORDER BY p.created_at DESC LIMIT ?
  `, [category, limit]),

  update: (id, data) => db.query(
    'UPDATE products SET title=?, description=?, category=?, price=?, `condition`=?, status=? WHERE id=?',
    [data.title, data.description, data.category, data.price, data.condition, data.status || 'available', id]
  ),

  deleteImages: (productId) => db.query('DELETE FROM product_images WHERE product_id = ?', [productId]),
  delete: (id) => db.query('DELETE FROM products WHERE id=?', [id]),
  markAsSold: (id) => db.query("UPDATE products SET status='sold' WHERE id=?", [id]),
  toggleFeatured: (id, val) => db.query('UPDATE products SET is_featured=? WHERE id=?', [val, id]),
